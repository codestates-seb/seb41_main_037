...
import GsPagination from "../../components/Pagination/GsPagination";
  ...
  useEffect(() => {
    ...
    if (data) {
      if (pageNum) {
        axios
          .get(
            `http://43.201.135.238:8080/products/search?key=&category=GS&page=${pageNum}&size=8`
          )
          .then((res) => {
        ...
        axios
          .get(
            `http://43.201.135.238:8080/products/search?key=&category=GS&page=0&size=8`
          )
          .then((res) => {
            setProducts(res.data.data);
            setPage(res.data.pageInfo.page);
